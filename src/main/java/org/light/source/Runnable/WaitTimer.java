...
public class WaitTimer extends BukkitRunnable {
    ...
    public void run() {
        ...
        if (!GameManager.getInstance().canStart() || GameManager.getInstance().getUsers().size() < DataManager.getInstance().getMinimumUser()) {
            if (countValue <= 0) {
                ...
                GameManager.getInstance().getUsers().forEach(data -> {
                    ...
                    if (!target.getWorld().getName().equalsIgnoreCase(DataManager.getInstance().getLocations()[0].getWorld().getName()))
        ...
        if (DataManager.getInstance().getLocations()[GameManager.getInstance().getRandomNumber()] != null) {
            if (countValue % 5 != 0)
